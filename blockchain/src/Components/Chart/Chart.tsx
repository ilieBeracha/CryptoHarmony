...
function Chart({
  ...
  useEffect(() => {
    ...
    const fetchHistoricalData = async () => {
      try {
        series.current = chartInstance.current.addLineSeries({
          title: "Market Cap",
          color: "blue",
        });
        ...
        const rsiDataPoints = rsiData.map((value, index) => ({
          time: data.prices[index][0] / 1000,
          value: value,
        }));
        const rsiSeries = chartInstance.current.addLineSeries({
          title: "RSI",
          color: "red",
          priceScaleId: "right",
        });
